...
INVALID_LICENSE_EXPRESSION_ERROR = "Invalid license expression"
...
def validate_license(component: dict, error_manager: ErrorManager) -> None:
    ...
    for component_license in component["licenses"]:
        ...
        if not valid_license:
            ...
            if not valid_license:
                details = "; ".join(licensing_validate.errors)
                error_manager.append_full_error_message(
                    f"{INVALID_LICENSE_EXPRESSION_ERROR}: {expression}. {details}"
                )
                return

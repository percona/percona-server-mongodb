...
# Creates a release-specific SBOM from a more general major-branch-specific one.
generate_release_sbom() {
    ...
    local branch_version="8.0"
    local release_version="${PSM_VER}-${PSM_RELEASE}"

    jq --indent 2 \
        --arg uuid "$(uuidgen --random)" \
        ...
        '.serialNumber = "urn:uuid:\($uuid)" | .version = 1 | .metadata = {
            ...
            "supplier": {"name": "Percona LLC", "url": ["https://www.percona.com"]}
        } | (.dependencies[] | select(.ref == $purl_branch)).ref |= $purl_release' "$src" > "$dst" \
        || abort '`generate_release_sbom`: `jq` failed'
}
...
build_tarball(){
    ...
    function check_libs {
        local elf_path=$1
        for elf in $(find ${elf_path} -maxdepth 1 -exec file {} \; | grep 'ELF ' | cut -d':' -f1); do
            if ! ldd ${elf}; then abort '`ldd` failed'; fi
        done
    }

...
(function () {
    ...
    const failPointAfterFetch = configureFailPoint(newNode, "initialSyncHangAfterFetchingStorageMetadataFile");
    ...
    assert(fileExists(primaryStorageMetadataPath), "primary is missing storage.bson: " + primaryStorageMetadataPath);
    assert(
        fileExists(stagedStorageMetadataPath),
        "FCBIS did not stage storage.bson under .initialsync before switching storage: " + stagedStorageMetadataPath,
    );
    assert.eq(
        md5sumFile(primaryStorageMetadataPath),

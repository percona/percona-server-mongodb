...
const verifyKeys = (keys, issuer_url, expectedNumberOfKeys) => {
    assert.eq(
        keys?.keySets[issuer_url]?.keys?.length,
...
function test_refresh_keys_response_and_log_on_failure(clusterClass) {
    ...
    assert.sameMembers(resp.failures || [], [
        expectedFailure(issuer_url1),
        expectedFailure(issuer_url2),
        expectedFailure(issuer_url3),
    ]);
    // note: in the case of a sharded cluster, the `resp` object contains additional
    // sharding-related properties

    assert(
        test.checkLogExists(expectedFailureLogEntry(issuer_url1), /* advanceLogCutOffTimePoint = */ false),
        "Expected log not found for issuer_url1",
    );
    assert(
        test.checkLogExists(expectedFailureLogEntry(issuer_url2), /* advanceLogCutOffTimePoint = */ false),
        "Expected log not found for issuer_url2",
    );
    assert(
        test.checkLogExists(expectedFailureLogEntry(issuer_url3), /* advanceLogCutOffTimePoint = */ false),

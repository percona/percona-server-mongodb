...

{
    ...
    specialColls.forEach((collName) => {
        assert.commandFailedWithCode(
            db.adminCommand({shardCollection: collName, key: {_id: 1}}),
    ...
    assert.commandFailedWithCode(
        db.adminCommand({shardCollection: "local.foo", key: {_id: 1}}),
    ...
    if (
        ...
        if (!TestData.shardsAddedRemoved) {
            assert.commandWorked(db.adminCommand({shardCollection: "config.system.sessions", key: {_id: 1}}));
        }
    }
}
...
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {aKey: "hahahashed"}}));
...
assert.commandWorked(db.adminCommand({shardCollection: kDbName + ".shard_key_dotted_path", key: {"_id.a": 1}}));
...
assert.commandWorked(
    db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, collation: {locale: "simple"}}),
);
...
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, unique: true}));
...
assert.commandWorked(
    db.adminCommand({shardCollection: kDbName + ".foo", key: {x: 1}, unique: true, enforceUniquenessCheck: false}),
);
...
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {aKey: "hashed"}, unique: true}));

jsTestLog("If shardCollection has unique:true it  must have a unique index.");
assert.commandWorked(db.foo.createIndex({aKey: 1}));

assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {aKey: 1}, unique: true}));
...
assert.commandWorked(sessionDb.adminCommand({shardCollection: kDbName + ".foo", key: {_id: "hashed"}}));
...
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, collation: true}));

jsTestLog('shardCollection should fail when the "collation" option cannot be parsed.');
assert.commandFailed(
    db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, collation: {locale: "unknown"}}),
);

jsTestLog('shardCollection should fail when the "collation" option is valid but is not the simple collation.');
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, collation: {locale: "en_US"}}));

jsTestLog('shardCollection should succeed when the "collation" option specifies the simple collation.');
assert.commandWorked(
    db.adminCommand({shardCollection: kDbName + ".foo", key: {_id: 1}, collation: {locale: "simple"}}),
);
...
assert.commandWorked(db.foo.createIndex({a: 1}));
assert.commandFailed(db.adminCommand({shardCollection: kDbName + ".foo", key: {a: 1}, collation: {locale: "simple"}}));

jsTestLog("shardCollection should succeed on an empty collection with a non-simple default collation.");
db.foo.drop();
assert.commandWorked(db.createCollection("foo", {collation: {locale: "en_US"}}));
assert.commandWorked(db.adminCommand({shardCollection: kDbName + ".foo", key: {a: 1}, collation: {locale: "simple"}}));
...
jsTestLog('shardCollection() propagates the value for "collation". Currently only the simple collation is supported.');
db.foo.drop();
assert.commandWorked(db.createCollection("foo"));

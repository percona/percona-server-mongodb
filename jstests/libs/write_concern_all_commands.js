...
const wcCommandsTests = {
            ...
            setupFunc: (coll, cluster, clusterType, secondariesRunning, optionalArgs) => {
                ...
                if (clusterType == "sharded" && bsonWoCompare(getShardKey(coll, fullNs), {}) == 0) {
                    ...
                    assert.commandWorked(coll.getDB().adminCommand(
                        {movePrimary: dbName, to: cluster.shard0.shardName}));
                }
    ...
    bulkWrite: {skip: "behind feature flag"},
    ...
    recipientSyncData: {skip: "internal command"},
    recipientVoteImportedFiles: {skip: "unused command"},
    ...
    waitForOngoingChunkSplits: {skip: "does not accept write concern"},
...
const wcTimeseriesViewsCommandsTests = {
    ...
    reIndex: {skip: "does not accept write concern"},
    reapLogicalSessionCacheNow: {skip: "does not accept write concern"},
    recipientForgetMigration: {skip: "unused command"},
    recipientSyncData: {skip: "unused command"},
    recipientVoteImportedFiles: {skip: "unused command"},
    refineCollectionShardKey: {
        ...
        success: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    // The two phase write path returns WriteConcernFailed in the write errors
                    // array.
                    assert.commandWorkedIgnoringWriteErrorsAndWriteConcernErrors(res);
                    assert(res.writeErrors && res.writeErrors.length == 3);
                    res.writeErrors.forEach((err) => {
...
function runCommandTest(
    ...
    try {
        ...
        let cmdName = Object.keys(request)[0];
        jsTestLog(tojson(cmdName) + " command returned " + tojson(res));
...
function shouldSkipTestCase(clusterType,
                            command,
                            testCase,
                            shardedCollection,
                            writeWithoutSk,
    ...
    // Cannot downgrade FCV when using a config shard because this feature does not exist on v6.0.
...
function executeWriteConcernBehaviorTests(conn,
    ...
    commandsToRun.forEach((command) => {
        ...
        if (cmd.noop) {
            if (!shouldSkipTestCase(clusterType,
                                    command,
                                    "noop",
                                    shardedCollection,
                                    writeWithoutSk,
...
export function checkWriteConcernBehaviorForAllCommands(
    ...
    const res = assert.commandWorked(cluster.configRS.getPrimary().adminCommand(
        {getParameter: 1, "failpoint.clusterAllCollectionsByDefault": 1}));
    const clusteredColls = res["failpoint.clusterAllCollectionsByDefault"].mode;

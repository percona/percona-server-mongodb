...
(function () {
    ...
    // ------------------------------------------------------------------
    {
        ...
        assert.commandWorked(adminDb.adminCommand({setParameter: 1, ldapServers: TestData.ldapServers}));
        adminDb.logout();
    ...
    // ------------------------------------------------------------------
    {
        ...
        assert(!db.auth({user: username, pwd: userpwd, mechanism: "PLAIN"}), "Auth should fail with 0ms ldapTimeoutMS");

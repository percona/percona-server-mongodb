...
(function () {
    ...
    response = assert.commandWorked(db.runCommand({killCursors: "$cmd.aggregate", cursors: [cursorId]}));
    ...
    response = assert.commandWorked(db.runCommand({killCursors: "$cmd.aggregate", cursors: [response.cursor.id]}));
    assert.eq(1, response.cursorsKilled.length);

    // Set a failpoint which will generate a uassert after the backup cursor is open.
    assert.commandWorked(db.adminCommand({configureFailPoint: "backupCursorErrorAfterOpen", mode: "alwaysOn"}));
    assert.commandFailed(db.runCommand({aggregate: 1, pipeline: [{$backupCursor: {}}], cursor: {}}));
    assert.commandWorked(db.adminCommand({configureFailPoint: "backupCursorErrorAfterOpen", mode: "off"}));

...
giving PSMDB developers the same access to the clone as they have to the PSMDB
repository itself.
...
```shell
$ pushd src/third_party/libarchive/dist
$ git diff --relative > ../patches/0001_<short_description>.patch
$ popd
```
...
```
That will apply patches in their numeric order. After committing the patches and
the updated `import.sh` into the PSMDB repository, one can reimport the library
and apply the patches in one go by removing the `dist` directory and running
`import.sh` again. Please see `src/third_party/abseil-cpp/scripts/import.sh`
as an example.
...
(venv) $ python buildscripts/tests/sbom_linter/sbom_linter.py --input-file sbom.json
```
...
2026-02-09 08:45:03,604 - INFO - sbom.json JSON data loaded.
2026-02-09 08:45:03,610 - INFO - Markdown file created successfully.

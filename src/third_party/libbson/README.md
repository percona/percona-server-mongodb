...
version 1.30.10, but did so only for `linux` on `amd64` and `arm64`
...
platforms from the repository. We also updated the `BUILD.bazel` file so that
it no longer mentions those platforms.

Since the `import.sh` script doesn't generate the platform-specific files
itself, we did it manually as follows:
```shell
$ pushd /tmp
$ git clone --recurse-submodules --shallow-submodules  --depth 1 --branch 1.30.10 \

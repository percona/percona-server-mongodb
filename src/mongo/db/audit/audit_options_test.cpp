...
protected:
    ...
    static constexpr auto TempDirPrefix = "ValidateAuditOptionsTestFixture";

    void setUp() override {
        // Reset audit options and set up default server parameters
        auditOptions = AuditOptions();

        _tmpDir = std::make_unique<unittest::TempDir>(TempDirPrefix);
        _cwd = getCwd();
...
TEST_F(ValidateAuditOptionsTestFixture, DestinationFile_FormatBSON_DefaultPathFromLog) {
    ...
    unittest::TempDir tmpDir(TempDirPrefix);
    const auto tmpDirPath = fs::path(tmpDir.path());
    serverGlobalParams.logpath = (tmpDirPath / "log.log").string();

    ASSERT_OK(storeAuditOptions(env));
    ASSERT_OK(validateAuditOptions());
    ASSERT_EQ(auditOptions.path, tmpDirPath / DefaultPathBson);
    ASSERT_TRUE(fs::exists(tmpDirPath / DefaultPathBson));
}
...
TEST_F(ValidateAuditOptionsTestFixture, DestinationFile_FormatJSON_DefaultPathFromLog) {
    ...
    ASSERT_OK(storeAuditOptions(env));
    ASSERT_OK(validateAuditOptions());
    ASSERT_EQ(auditOptions.path, tmpDirPath / DefaultPathJson);
    ASSERT_TRUE(fs::exists(tmpDirPath / DefaultPathJson));
}

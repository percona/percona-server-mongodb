...
struct KeyDBAPI {
    ...
    /**
     * Delete a specific encryption key by its ID (database name).
     * Returns true if the key was successfully deleted, false otherwise.
     */
    virtual bool deleteEncryptionKey(const std::string& keyId) {
        return false;  // no-op for engines which do not support KeyDB
    }

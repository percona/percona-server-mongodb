...
void validate(const OidcIdentityProvidersServerParameter& param) {
    // A vector of indexes of those `OidcIdentityProviderConfig`s that have
    // enabled `supportsHumanFlow` but unspecified `matchPattern`.
    std::vector<std::size_t> noMatchPatternIndexes;
    ...
    for (std::size_t i = 0; i < param._data.size(); ++i) {
        ...
        validate(conf, i);
        if (conf.getSupportsHumanFlows() && !conf.getMatchPattern()) {
            noMatchPatternIndexes.push_back(i);
        }
        equalAudienceIndexes[{conf.getIssuer(), conf.getAudience()}].push_back(i);
    }

    uassert(77703,
            errorMsgHeader(noMatchPatternIndexes)
                << "`supportsHumanFlow` enabled but there is no `matchPattern`. "
                << "If more that one configuration has `supportsHumanFlow` enabled, "
                << "then all such configurations must have `matchPattern`.",
            noMatchPatternIndexes.size() < 2);
...
Status paramDeserialize(OidcIdentityProvidersServerParameter& param, const BSONArray& arr) {
...
Status OidcIdentityProvidersServerParameter::set(const BSONElement& elem,
    ...
    return paramDeserialize(*this, BSONArray(elem.Obj()));
}
...
Status OidcIdentityProvidersServerParameter::setFromString(StringData str,
    ...
    uassert(ErrorCodes::TypeMismatch,
            ...
            isArray(str));
    return paramDeserialize(*this, BSONArray(fromjson(str)));
}
}  // namespace mongo

...
StatusWith<std::tuple<bool, std::string>> SaslOidcServerMechanism::step2(
    ...
    if (idp->getUseAuthorizationClaim()) {
        auto status = processAuthorizationClaim(*idp, token);
        if (!status.isOK()) {
            return status;
        }
    }
...
Status SaslOidcServerMechanism::processAuthorizationClaim(const OidcIdentityProviderConfig& idp,
    ...
    uassert(ErrorCodes::BadValue,
            ...
            token.getBodyBSON().hasField(authorizationClaim));
    ...
    const auto _authClaim = token.getBodyBSON().getField(authorizationClaim);

...
// which updates cached $external users with changed roles from the backend.
TEST_F(AuthorizationManagerTest, testLDAPCacheInvalidatorRunRefreshPath) {
    ...
    // Acquire user to populate the cache.
    auto swUser = authzManager->acquireUser(
        opCtx.get(),
    ...
    {
        RoleNameIterator roles = cachedUser->getRoles();
        ASSERT_EQUALS(RoleName("read", "test"), roles.next());
        ASSERT_FALSE(roles.more());
    }

    // Update the backend so the external user now has a different role.
    ASSERT_OK(
        mockBackend->updateOne(opCtx.get(),
                               NamespaceString::kAdminUsersNamespace,
                               BSON("user" << "extUser"),
    ...
    // The old cached handle should be invalid (refreshExternalUsers replaces the cache entry).
    ASSERT(!cachedUser.isValid());

    // Re-acquire and verify that the refreshed cache has the updated role.
    swUser = authzManager->acquireUser(
        opCtx.get(),
        std::make_unique<UserRequestGeneral>(UserName("extUser", "$external"), boost::none));
    ASSERT_OK(swUser.getStatus());
    auto refreshedUser = std::move(swUser.getValue());

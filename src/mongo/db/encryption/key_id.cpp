...
bool canReplaceAdopted(const std::optional<std::uint64_t>& current,
                       const std::optional<std::uint64_t>& lastAdopted) {
...
public:
    AdoptIntoEncryptionParams(std::string& lastKmip,
                              ...
                              std::optional<std::uint64_t>& lastVaultVersion)
        : _lastKmip(lastKmip),
...
private:
    ...
    std::optional<std::uint64_t>& _lastVaultVersion;
};
}  // namespace

 ...
 * - Shows PR links (open/merged/closed) or a "no PR" indicator when none exist
...
(function () {
  ...
  async function fetchPRsWithCache() {
    ...
    // Only cache successful results (non-empty array)
    if (prs.length > 0) {
      try {
  ...
  function updatePRStatus(element, commitSha, pr) {
    ...
    if (pr) {
      ...
      anchor.href = pr.html_url;
      anchor.target = "_blank";
      anchor.className = `pr-link ${stateInfo.class}`;
      anchor.title = `${stateInfo.label}: ${pr.title}`;
      anchor.textContent = `${stateInfo.icon} #${pr.number}`;

      element.appendChild(anchor);
    } else {
      // No PR found - leave empty (user can copy SHA and trigger workflow manually)
      const span = document.createElement("span");
      span.className = "pr-none";
      span.title = "No PR found";
      span.textContent = "—";
